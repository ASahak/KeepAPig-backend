...
export class SendgridService {
  ...
  public send({ email, clientOrigin }: { email: string, clientOrigin: string }): Observable<boolean> {
    return this.userService.doesUserExist({ email }, true).pipe(
      switchMap((user: User) => {
        if (!!user) {
          ...
          return from(this.userService.updateUser(user._id, { resetPasswordToken })).pipe(
            switchMap(_ => {
              let link = `${clientOrigin}?token=${resetPasswordToken}`;
              ...
              return source.pipe(
                map(_ => true),
                catchError(_ => {
                  throw new HttpException(MESSAGES.HTTP_EXCEPTION.SENDGRID_FAILURE, HttpStatus.FAILED_DEPENDENCY)
                })
              );
            })
          );
